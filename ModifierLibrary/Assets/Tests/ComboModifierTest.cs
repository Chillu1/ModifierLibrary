...
namespace ModifierLibrary.Tests
{
	public class ComboModifierTest : ModifierBaseTest
	{
		...
		public void ComboModifierByElements()
		{
			...
			character.AddModifierWithParameters(poisonAttackApplier,
				AddModifierParameters.CheckRecipes | AddModifierParameters.NullStartTarget);
			character.AddModifierWithParameters(bleedAttackApplier,
				AddModifierParameters.CheckRecipes | AddModifierParameters.NullStartTarget);
		...
		public void ComboModifierDoTInit()
		{
			...
			Assert.AreEqual(initialHealthEnemy,
				enemy.Stats.Health.CurrentHealth + 1 + 2 * 2 + 10 * Curves.ComboElementMultiplier.Evaluate(5));
		}
