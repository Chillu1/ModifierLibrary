	...
	public class ModifierController : ISavable
	{
		...
		public bool TryAddModifier(Modifier modifier, AddModifierParameters parameters, Unit sourceUnit = null)
		{
			...
			if (ContainsModifier(modifier, out Modifier internalModifier))
			{
				...
				if (!stacked && !refreshed /* && internalModifier.RepeatInitEffect*/
				   ) //Aura effects shouldn't be applied multiple times, if we didn't make a cast applier refreshable, this will trigger again, needs to be fixed//TODO
