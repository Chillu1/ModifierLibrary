...
namespace ModifierSystem.Sample
{
    public class BeingController
    {
        ...
        public BaseBeing SpawnBaseBeing(BeingProperties properties, bool isEnemy)
        {
            ...
            if (isEnemy)
            {
                being.TargetingSystem.SetupTargets(GetRandomCharacterTarget, GlobalRandom.NextFloat(0.05f, 0.3f));
                _enemies.Add(being);
            }
            else
            {
                being.TargetingSystem.SetupTargets(GetRandomEnemyTarget, GlobalRandom.NextFloat(0.05f, 0.3f));
